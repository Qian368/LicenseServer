...
-   **智能许可证管理**：验证许可证状态、有效期，管理设备绑定数量。当单设备多绑或产生脏数据时，自动提取“最长有效期、永久、激活状态”的最优许可证，并静默清理多余记录。
-   **无感自动恢复（免密登录）**：如果当前机器码曾在服务器成功绑定过，用户即使不输入密钥，系统也能自动从服务器查询绑定的密钥并完成验证登录。
-   **本地授权缓存**：验证成功后生成加密的本地授权文件，存储于系统级公共数据目录 (`C:\ProgramData\LicenseServer\AppName`)，彻底避免权限问题和多用户数据隔离问题。后续启动优先校验本地文件。
-   **命令行与管道通信驱动**：支持通过命令行参数或**匿名管道** (`-pipemode`) 控制验证流程，子程序完全解耦并以无窗口模式为其他语言/应用提供验证服务。
-   **对外 API**：提供 `RemoteVerifyApi` 方法，供同处于 .NET 环境的应用程序直接调用。

## 快速开始

- **调用方式**：也支持通过 GUI 界面操作，同时也支持通过命令行参数调用程序的所有功能、验证模式支持匿名管道通信调用。具体调用方法详见接口文档。
...
| `-LicenseKey` | 否 | 要验证的许可证密钥。如果留空，程序会尝试**使用本机机器码免密自动恢复**绑定的许可证。若均失败才提示要求输入。 |
| `-Port` | 否 | 设置本地PocketBase服务的HTTP端口，默认为 `8090`。 |
| `-ApiUrl` | 否 | **（优先级最高）** 指定远程验证API的完整根地址 (如：`http://192.168.1.100:8090`)。 |
| `-LimitDate` | 否 | 设置许可证有效期剩余多少天时触发提醒，默认为 `30` 天。 |
| `-SilentMode` | 否 | 启用静默模式。程序主窗口将完全隐藏，通过控制台或日志输出结果。 |
| `-PipeMode` | 否 | **(开发集成专用)** 开启匿名管道通信模式。后接读写句柄，直接作为子进程承接第三方主程序的验证请求。 |
